Feature: Kafka Test Suite3
  ...
  Scenario: Creating Consumer
    ...
    Then status 200
    * print response
    * match response.instance_id == "syed_consumer"
  ...
  Scenario: Subscribing to topic
    Given path "/consumers/my-consumer-group/instances/syed_consumer/subscription"
    And request
      """
      {
        "topics" : [ "syed_topic" ]
  ...
  Scenario: Getting Subscription
    ...
    Then status 200
    * print response
    * assert response.topics[0] == "syed_topic"


  Scenario: Getting Subscription records
    Given path "/consumers/my-consumer-group/instances/syed_consumer/records"
  ...
  Scenario: Getting assigned Assignments
    Given path "/consumers/my-consumer-group/instances/syed_consumer/assignments"
    And header Accept = "application/vnd.kafka.v2+json"
    When method get
    Then status 200
    * print response
    * assert response.partitions[0].topic == "syed_topic"
  ...
  Scenario: Deleting consumer
    Given path "/consumers/my-consumer-group/instances/syed_consumer"
    And header Accept = "application/vnd.kafka.v2+json"
    When method delete
    Then status 204
